...
const ShopForm = ({
  ...
  // Previously: `const mallId = User.mallId;` at module scope referenced
  // an undefined global `User` (not imported, not the logged-in user) —
  // this threw a ReferenceError the instant this file was loaded,
  // crashing the entire Shops section of the app. mallId now comes from
  // the active-mall context (axiosClient also auto-attaches it to every
  // request, but the form needs it for display/validation purposes too).
...
export const EditShop = () => {
  ...
  const { data: shop, loading } = useFetch(async () => {
    try {
      return await shopService.getById(id);
    } catch {
      return SEED_SHOP;
    }
  }, [id]);
...
export const ShopDetails = () => {
  ...
  return (
    <>
      <PageHeader
        title={`Shop #${shop?.shopNumber}`}
        breadcrumbs={[
          { label: "Shops", to: ROUTES.SHOPS },
          { label: `Shop #${shop?.shopNumber}` },
        ]}
        actions={
          <Button onClick={() => navigate(ROUTES.SHOP_EDIT.replace(":id", id))}>
            Edit
          </Button>
        }
      />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="Shop Details">
          {[
            ...
            { label: "Size", value: shop?.size ? `${shop.size} sqft` : "—" },
            {
              label: "Monthly Rent",
              value: shop?.monthlyRent
            ...
            <div
              key={label}
              className="flex justify-between border-b border-gray-50 dark:border-gray-800 py-2.5 last:border-0"
            >
